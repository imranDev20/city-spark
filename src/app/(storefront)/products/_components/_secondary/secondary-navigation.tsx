...
export default function SecondaryNavigation({data,primaryName}:{data:CategoryWithRelation[], primaryName:string}) {
        ...
        {data?.map((values:any, index:any) => (
          ...
          <li key={values.id}>
            <div className="flex justify-between items-center px-6 py-3">
              <Link href={`/products/${primaryName}/${convertToKebabCase(values.name)}`}>
                <p className={`${openCategory === values.id ? 'font-semibold' : 'font-normal'} text-[16px] py-3 ps-6 cursor-pointer`}>
                  {values.name}
                </p>
              </Link>
              {values.secondaryChildCategories.length > 0 && (
                <div
            ...
            {openCategory === values.id && (
              <ul className="ml-4 -mt-5">
                {values.secondaryChildCategories.map((subcategory : any) => (
