...
export default function ProductTableHeader() {
  ...
  const handleDownload = async (format: BackupFormat) => {
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          if (format === "JSON") {
            ...
            filename = `Products_Backup_${dayjs().format(
              "YYYY-MM-DD@HH:mm:ss"
            )}.json`;
          } else {
          ...
          toast({
            title: `Products ${format} Downloaded`,
            description:
        ...
        toast({
          title: `Products ${format} download failed`,
          description:
